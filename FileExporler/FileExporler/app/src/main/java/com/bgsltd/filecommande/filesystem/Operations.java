package com.bgsltd.filecommande.filesystem;
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                if (f.isSmb()) {
                    ...
                    File fileSource = new File(f.getPath());
                    File fileTarget = new File(f1.getPath());
                        switch (f.getMode()){
                            case HFile.LOCAL_MODE:
                                int mode = checkFolder(fileSource.getParentFile(), context);
                                if (mode == 2) {
                                    errorCallBack.launchSAF(f,f1);
                                } else if (mode == 1 || mode==0) {
                                    boolean b = FileUtil.renameFolder(fileSource, fileTarget, context);
                                    boolean a = !fileSource.exists() && fileTarget.exists();
                                    if (!a && rootMode){
                                        try {
                                            renameRoot(fileSource, fileTarget.getName());
                                        } catch (Exception e) {
                                            Logger.log(e,f.getPath()+"\n"+f1.getPath(),context);
                                        }
                                       f.setMode(HFile.ROOT_MODE);
                                      f1.setMode(HFile.ROOT_MODE);
                                      a = !fileSource.exists() && fileTarget.exists();
                                    }
                                    errorCallBack.done(f1,fileTarget.exists());
                                    return null;
                                }
                                break;
