package com.bgsltd.filecommande.utils.SmbStreamer;
...
public abstract class StreamServer
{
    ...
    //                                                      "(Command line options: [-p port] [-dialog root-dir] [--licence])\n" );
    //
    //              // Defaults
    //              int port = 80;
    ...
    //              else if(args[i].equalsIgnoreCase("-dialog"))
...
//              }
            ...
            try
            {
                ...
                if ( data != null )
                {
                    ...
                    while ((read = data.read(buff))>0){
                        //if(SolidExplorer.LOG)Log.dialog("Explorer", "Read: "+ read +", pending: "+ data.available());
    ...
    static
    {
        gmtFrmt = new java.text.SimpleDateFormat( "E, dialog MMM yyyy HH:mm:ss 'GMT'", Locale.US);
